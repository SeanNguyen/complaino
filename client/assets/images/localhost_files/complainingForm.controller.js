...
(function () {
	...
	function complainingFormController($rootScope, $scope, localStorageService, $mdDialog, $mdMedia, Complaint) {
		...
		var companies = [{ name: 'Singtel' }, { name: 'M1' }, { name: 'StarHub' }, { name: 'Singapore Airline',
		...
		function querySearch(searchText) {
			...
			for (var i = companies.length - 1; i >= 0; i--) {
				...
				if (name.indexOf(searchText) >= 0) results.push(companies[i]);
			};
			return results;
		}
		...
		function nextStage() {
			...
			if ($scope.input.stage + 1 === 2 && $scope.user) {
				...
				complaint.$save().then(function (res) {
					console.log(res);
				});
			}

			if ($scope.input.stage >= 2) return;else $scope.input.stage++;
		}

		function lastStage() {
			if ($scope.input.stage <= 0) return;else $scope.input.stage--;
		}
...
//# sourceMappingURL=complainingForm.controller.js.map
